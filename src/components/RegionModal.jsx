...
export default function RegionModal({ isModalActive, setIsModalActive, setRegion }) {
  ...
  return (
    <div onClick={closeModal} className={isModalActive ? 'z-50 fixed w-full h-full left-0 top-0 flex bg-transparent items-center backdrop-blur-sm justify-center duration-200 visible' : 'z-50 invisible duration-200 fixed w-full h-full left-0 top-0 flex bg-transparent items-center justify-center'}>
        <div onClick={(e) => {e.stopPropagation()}} className={isModalActive ? 'pt-10 relative z-10 w-[500px] m600:w-[290px] bg-white dark:bg-black rounded-lg opacity-100 visible' : 'invisible opacity-50 z-10 rounded-lg'}>
